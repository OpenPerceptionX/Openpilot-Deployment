...
void model_init_extra(ModelState* s, cl_device_id device_id, cl_context context) {
  s->frame = new ModelFrame(device_id, context);

  s->m = std::make_unique<ExtraModel>("../../models/supercombo_extra.onnx", &s->output[0], NET_OUTPUT_SIZE, USE_GPU_RUNTIME, std::string("/runners/onnx_runner_extra.py"));
...
#elif USE_ONNX_MODEL
  s->m = std::make_unique<ONNXModel>("../../models/supercombo.onnx", &s->output[0], NET_OUTPUT_SIZE, USE_GPU_RUNTIME, std::string("/runners/onnx_runner.py"));
#else
...
void fill_model(cereal::ModelDataV2::Builder &framed, const ModelOutput &net_outputs) {
  ...
  fill_plan(framed, best_plan_extra);
