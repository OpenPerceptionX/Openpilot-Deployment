...
def run_loop(m):
  ...
  print("ready to run extra onnx model", keys, ishapes, file=sys.stderr)
  while 1:
    inputs = []
    for shp in ishapes:
      ts = np.product(shp)
...
if __name__ == "__main__":
  ...
  print("Extra Onnx available providers: ", ort.get_available_providers(), file=sys.stderr)
  ...
  print("Extra Onnx selected provider: ", [provider], file=sys.stderr)
  ort_session = ort.InferenceSession(sys.argv[1], options, providers=[provider])
  print("Extra Onnx using ", ort_session.get_providers(), file=sys.stderr)
